...
const User = mongoose.model('teas',{
    loginName: String,
    realName: String,
    password: String,
...
//    验证省略
    ...
    User.findByIdAndUpdate(req.body.id,updateStatement,function (err,u) {
        if (err){
            res.json({code: 'error',message:"保存失败"});
            return;
        }
